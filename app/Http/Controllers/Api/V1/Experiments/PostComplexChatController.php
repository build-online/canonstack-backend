...
class PostComplexChatController extends Controller
{
    ...
    /**
     * Send a message in the complex chat for this dataset.
     */
    public function __invoke(Request $request, string $uuid): JsonResponse
    {
        try {
            // Find dataset and load complex embedding
            $dataset = Dataset::where('uuid', $uuid)->with('repository')->firstOrFail();
            $embedding = $dataset->getEmbeddingByVariant('complex');

            if (!$embedding) {
                return response()->sendError('Complex embedding not found for this dataset.', 404);
            }

            if (!$embedding->isCompleted()) {
                return response()->sendError('Complex embedding is not completed yet. Please wait for generation to finish.', 403);
            }
            ...
            Log::info("Complex chat message sent", [
                'dataset_uuid' => $uuid,
                'user_id' => $user->id,
            ...
            return response()->sendResponse(
                $result,
                null,
                'Complex chat message processed successfully.'
            );

        } catch (Exception $e) {
            Log::error("Complex chat message failed", [
                'dataset_uuid' => $uuid,
            ...
            if (str_contains($e->getMessage(), 'embeddings are not available')) {
                return response()->sendError('Complex embeddings are not available. Please generate them first.', 400);
            }

            return response()->sendError('Failed to process complex chat message: ' . $e->getMessage(), 500);
        }
    }
}
