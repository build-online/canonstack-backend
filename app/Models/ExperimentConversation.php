...
class ExperimentConversation extends Model
{
    ...
    {
        return $this->hasMany(ExperimentConversationMessage::class)->orderBy('created_at');
    }
    ...
    {
        return $this->hasMany(ExperimentConversationMessage::class)->latest();
    }
    ...
    public function getContextForAI(): array
    {
        return $this->messages()
            ->select(['role', 'summarized_content', 'created_at'])
            ->get()
            ->map(function ($message) {
                return [
                    'role' => $message->role,
                    'content' => $message->summarized_content,
        ...
        return $this->messages()
            ...
            ->map(function ($message) {
                return [
                    'id' => $message->id,
                    'role' => $message->role,
                    'content' => $message->original_content ?? $message->summarized_content,
                    'summarized_content' => $message->summarized_content,
    ...
    /**
     * Find or create conversation for user, dataset embedding, and variant.
     */
    public static function findOrCreateForUser(int $userId, int $datasetEmbeddingId, string $variant): self
    {
        ...
        try {
            ...
            if (!$conversation) {
                throw new \Exception("Failed to create experiment conversation - create() returned null");
            }
            ...
            throw new \Exception("Failed to create experiment conversation for user {$userId}, dataset embedding {$datasetEmbeddingId}, and variant {$variant}: " . $e->getMessage());
        }
    }
